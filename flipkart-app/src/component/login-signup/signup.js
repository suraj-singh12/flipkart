...
const url = "https://login-auth-k7a7.onrender.com/api/auth/register";
    ...
    handleSubmit = (event) => {
        ...
        fetch(url, {
            ...
            .then((data) => {
                ...
                if (data.status === 500) {
                    // user already exists
                    // Response {type: 'cors', url: 'https://login-auth-k7a7.onrender.com/api/auth/register', redirected: false, status: 500, ok: false, …}
                    message = "User Already Exists!! Please Login";
                }
                // user registered
                // Response {type: 'cors', url: 'https://login-auth-k7a7.onrender.com/api/auth/register', redirected: false, status: 200, ok: true, …}
                alert(message);
                this.props.history.push('/login');
            }); // redirecting to login, after user is registered.
        // for dev purpose, all registered users can be seen here: https://login-auth-k7a7.onrender.com/api/auth/users 
    }
